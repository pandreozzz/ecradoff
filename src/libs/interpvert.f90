MODULE mo_interp
...
#ifdef ECRADOFF_DP
  use iso_fortran_env, realk => real64,     intk => int32
  use iso_c_binding, c_realk => c_double, c_intk => c_int32_t
#else
  use iso_fortran_env, realk => real32,    intk => int32
  use iso_c_binding, c_realk => c_float, c_intk => c_int32_t
#endif
  ...
  SUBROUTINE interp(psrc, ptgt, tgtlevs, weights, &
    ...
    real(realk), intent(in), dimension(nlevsrc,nsrc,ncom) :: psrc
    real(realk), intent(in), dimension(nlevtgt,ntgt,ncom) :: ptgt

    integer(intk), intent(out) :: tgtlevs(nlevtgt,ntgt,nsrc,ncom)
    real(realk),     intent(out) :: weights(nlevtgt,ntgt,nsrc,ncom)
...
#ifdef ECRADOFF_DP
    write(*,*) "Running interp in double precision"
#else
    write(*,*) "Running interp in single precision"
#endif
    ...
    if (thread_id == 0) then
        ...
        !write(*,'(A,I0,A)') "Interp using ",num_threads," threads"
...
tgtlevloop: do ktgt=1,nlevtgt
 ...
 srclevloop:  do ksrc=ksrcstart,nlevsrc
  ...
  SUBROUTINE interp_fld(fsrc, fdst, tgtlevs, weights, &
    ...
    real(realk), intent(in),  dimension(nlevsrc,nsrc,ncom) :: fsrc
    real(realk), intent(out), dimension(nlevdst,ntgt,nsrc,ncom) :: fdst

    integer(intk), intent(in), dimension(nlevdst,ntgt,ncom) :: tgtlevs
    real(realk),      intent(in), dimension(nlevdst,ntgt,ncom) :: weights
    ...
    if (thread_id == 0) then
        num_threads = int(omp_get_num_threads(), intk)
        !write(*,'(A,I0,A)') "Interp fld using ",num_threads," threads"
  ...
  SUBROUTINE f_interp(psrc, ptgt, tgtlevs, weights, ncom, nsrc, ntgt, nlevsrc, nlevtgt, chunk_size_in) &
    ...
    integer(c_intk), intent(in), value  :: ncom, nsrc, ntgt
    integer(c_intk), intent(in), value  :: nlevsrc
    integer(c_intk), intent(in), value  :: nlevtgt

    real(c_realk), intent(in), dimension(nlevsrc,nsrc,ncom) :: psrc
    real(c_realk), intent(in), dimension(nlevtgt,ntgt,ncom) :: ptgt

    integer(c_intk), intent(out) :: tgtlevs(nlevtgt,ntgt,nsrc,ncom)
    real(c_realk),      intent(out) :: weights(nlevtgt,ntgt,nsrc,ncom)
  ...
  SUBROUTINE f_interp_fld(fsrc, fdst, tgtlevs, weights, ncom, nsrc, ntgt, nlevsrc, nlevtgt, chunk_size_in) &
    ...
    integer(c_intk), intent(in), value  :: ntgt, nsrc, ncom
    integer(c_intk), intent(in), value  :: nlevtgt, nlevsrc

    real(c_realk), intent(in),  dimension(nlevsrc,nsrc,ncom) :: fsrc
    real(c_realk), intent(out), dimension(nlevtgt,ntgt,nsrc,ncom) :: fdst

    integer(c_intk), intent(in) :: tgtlevs(nlevtgt,ntgt,nsrc,ncom)
    real(c_realk),      intent(in) :: weights(nlevtgt,ntgt,nsrc,ncom)
